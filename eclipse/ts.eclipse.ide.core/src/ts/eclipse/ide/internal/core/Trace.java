	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				TypeScriptCorePlugin
						.getDefault()
						.getLog()
						.log(new Status(IStatus.ERROR,
								TypeScriptCorePlugin.PLUGIN_ID, s, t));
				logged.add(s);
			}
		}

		if (!TypeScriptCorePlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(TypeScriptCorePlugin.PLUGIN_ID);
		sb.append(" ");
