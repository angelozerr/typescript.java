 ...
 */
package ts.eclipse.ide.server.nodejs.internal.core;
...
import ts.eclipse.ide.server.nodejs.core.TypeScriptNodejsCorePlugin;
	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				TypeScriptNodejsCorePlugin
						.getDefault()
						.getLog()
						.log(new Status(IStatus.ERROR,
								TypeScriptNodejsCorePlugin.PLUGIN_ID, s, t));
				logged.add(s);
			}
		}

		if (!TypeScriptNodejsCorePlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(TypeScriptNodejsCorePlugin.PLUGIN_ID);
		sb.append(" ");
