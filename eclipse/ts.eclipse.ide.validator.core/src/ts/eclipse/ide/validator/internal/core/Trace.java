 ...
 */
package ts.eclipse.ide.validator.internal.core;
...
import ts.eclipse.ide.validator.core.TypeScriptValidationCorePlugin;
	...
	public static void trace(byte level, String s, Throwable t) {
		...
		if (level == SEVERE) {
			if (!logged.contains(s)) {
				TypeScriptValidationCorePlugin
						.getDefault()
						.getLog()
						.log(new Status(IStatus.ERROR,
								TypeScriptValidationCorePlugin.PLUGIN_ID, s, t));
				logged.add(s);
			}
		}

		if (!TypeScriptValidationCorePlugin.getDefault().isDebugging())
			return;

		StringBuilder sb = new StringBuilder(TypeScriptValidationCorePlugin.PLUGIN_ID);
		sb.append(" ");
