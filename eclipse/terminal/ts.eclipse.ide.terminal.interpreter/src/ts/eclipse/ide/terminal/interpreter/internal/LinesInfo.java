...
public class LinesInfo {
	...
	public LinesInfo(byte[] byteBuffer, int bytesRead, String encoding) {
	...
	/**
	 * This method dispatches control to various processing methods based on the
	 * command character found in the most recently received DEC private mode
	 * escape sequence. This method only handles command characters that follow
	 * the control sequence CSI ?
	 */
	...
	private void processAnsiCommand_K() {
		// Bug 401386: missing parameter must be interpreted as 0, and not 1
		// like most other defaults.
		int ansiParameter = 0;
		if (ansiParameters[0].length() > 0)
			ansiParameter = getAnsiParameter(0);
	...
	private void processAnsiCommand_m() {
		...
		// // There are a non-zero number of ANSI parameters. Process each one
		// in
		// // order.
		//
		...
		// default:
		// Logger
		// .log("Unsupported graphics rendition parameter: " + ansiParameter);
		// //$NON-NLS-1$
		// break;
		// }
		//
	...
	private void processAnsiCommand_n() {
		...
		// // endpoint.
		// reply = "\u001b[" + (relativeCursorLine() + 1) + ";" + //$NON-NLS-1$
		// //$NON-NLS-2$
		...
		//
		// try {
		// terminal.getOutputStream().write(reply.getBytes("ISO-8859-1"));
		// //$NON-NLS-1$
	...
	private void displayNewText(String string) {
