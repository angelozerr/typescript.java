...
public class TypeScriptSearchResultPage extends AbstractTextSearchViewPage implements IAdaptable {
	...
	private ITypeScriptSearchContentProvider fContentProvider;
	...
	protected void fillContextMenu(IMenuManager mgr) {
		...
		TypeScriptSearchQuery query= (TypeScriptSearchQuery) getInput().getQuery();
...
//		}
	...
	protected void evaluateChangedElements(Match[] matches, Set changedElements) {
		if (showLineMatches()) {
			for (int i = 0; i < matches.length; i++) {
				changedElements.add(((TypeScriptMatch) matches[i]).getLineElement());
			}
		} else {
	...
	private boolean showLineMatches() {
		...
		return true;//return getLayout() == FLAG_LAYOUT_TREE && input != null && !((TypeScriptSearchQuery) input.getQuery()).isFileNameSearch();
	}
