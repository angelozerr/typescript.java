...
public class WorkspaceResourceSelectionDialog extends ElementTreeSelectionDialog {
	...
	protected boolean checkMode(Object selection) {
		return mode == Mode.FILE && selection instanceof IFile || mode == Mode.FOLDER && selection instanceof IFolder
	...
	protected Control createDialogArea(Composite parent) {
		...
		if (filters.length > 0 && !(filters.length == 1 && filters[0] instanceof PatternFilter)) {
			...
			final Button showAllFilesButton = new Button((Composite) dialogControl, SWT.CHECK);
			showAllFilesButton.setText("Show all files"); //$NON-NLS-1$
			GridDataFactory.fillDefaults().align(SWT.END, SWT.CENTER).grab(true, false).span(2, 2)
					.applyTo(showAllFilesButton);
