 ...
 *
 */
public abstract class AbstractAnnotationHover extends AbstractTypeScriptHover {
	...
	private static final class HoverControlCreator extends AbstractReusableInformationControlCreator {
		 ...
		 * AbstractReusableInformationControlCreator#canReuse(org.eclipse.jface.
		 * text.IInformationControl)
		 */
		@Override
	...
	private static class AnnotationInformationControl extends AbstractInformationControl
		...
		/*
		 * @see
		 * org.eclipse.jface.text.IInformationControl#setInformation(java.lang.
		 * String)
		 */
		@Override
		 ...
		 * AbstractInformationControl#setFocus()
		 */
		@Override
		...
		private void createCompletionProposalsList(Composite parent, ICompletionProposal[] proposals) {
			...
			for (int i = 0; i < proposals.length; i++) {
				list.add(createCompletionProposalLink(composite, proposals[i], 1));// Original
																					// link
																					// for
																					// single
																					// fix,
				...
				// (FixCorrectionProposal)proposals[i];
				// int count=
				// proposal.computeNumberOfFixesForCleanUp(proposal.getCleanUp());
				// if (count > 1) {
				// list.add(createCompletionProposalLink(composite,
				// proposals[i], count));
				// }
				// }
			}
		...
		private Link createCompletionProposalLink(Composite parent, final ICompletionProposal proposal, int count) {
			...
			Image image = isMultiFix ? null
					/*
					 * TODO JavaPluginImages.get(JavaPluginImages.
