...
public abstract class OptionsConfigurationBlock {
	...
	protected final static Key getTypeScriptCoreKey(String key) {
		return getKey(TypeScriptCorePlugin.PLUGIN_ID, key);
	}

	protected final static Key getTypeScriptUIKey(String key) {
		return getKey(TypeScriptUIPlugin.PLUGIN_ID, key);
	}

	private void testIfOptionsComplete(Key[] allKeys) {
		for (int i = 0; i < allKeys.length; i++) {
			if (allKeys[i].getStoredValue(fLookupOrder, false, fManager) == null) {
				TypeScriptUIPlugin.logErrorMessage(
	...
	private int getRebuildCount() {
		return fManager.getWorkingCopy(new DefaultScope().getNode(TypeScriptUIPlugin.PLUGIN_ID))
				.getInt(REBUILD_COUNT_KEY, 0);
	}

	private void incrementRebuildCount() {
		fRebuildCount++;
		fManager.getWorkingCopy(new DefaultScope().getNode(TypeScriptUIPlugin.PLUGIN_ID)).putInt(REBUILD_COUNT_KEY,
				fRebuildCount);
	}
	...
	protected Combo newComboControl(Composite composite, Key key, String[] values, String[] valueLabels) {
		return newComboControl(composite, key, values, valueLabels, true);
	}
	...
	protected void updateCombo(Combo curr) {
		if ((curr.getStyle() & SWT.READ_ONLY) != 0) {
