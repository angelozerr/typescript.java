...
public class TypeScriptAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	 ...
	 */
	public TypeScriptAutoIndentStrategy(String partitioning, ITypeScriptFile project, ISourceViewer viewer) {
...
//		}
	...
	private void smartPaste(IDocument document, DocumentCommand command) {
		...
		try {
			JavaHeuristicScanner scanner= new JavaHeuristicScanner(document);
			TypeScriptIndenter indenter= new TypeScriptIndenter(document, scanner, fProject);
			...
			indenter= new TypeScriptIndenter(temp, scanner, fProject);
	...
	private boolean isInsertingSpacesForTab() {
		return fProject.getFormatOptions().getConvertTabsToSpaces();//JavaScriptCore.SPACE.equals(getCoreOption(fProject, DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR));
	}
