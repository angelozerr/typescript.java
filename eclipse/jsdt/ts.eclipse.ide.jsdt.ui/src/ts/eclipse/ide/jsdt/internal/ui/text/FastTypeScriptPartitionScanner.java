...
/**
 * This scanner recognizes the JSDoc comments, multi line comments, single line
 * comments, strings, characters, and regular expressions.
 */
public class FastTypeScriptPartitionScanner implements IPartitionTokenScanner, IJavaScriptPartitions, IJSXPartitions {
	...
	private static final int BACKSLASH_CARRIAGE_RETURN = 8; // anti-postfix for
															// STRING, CHARACTER
	private static final int HASH = 9; // prefix for SHEBANG
	private static final int BACKQUOTE = 10; // prefix for JS_TEMPLATE_LITERAL
	private static final int DOLLAR = 11; // prefix for template literal
											// expression
	private static final int DOLLAR_CURLYBRACE_LEFT = 12; // prefix for template
															// literal
															// expression
	private static final int CURLYBRACE_RIGHT = 13; // postfix for template
													// literal expression
	...
	private final IToken[] fTokens = new IToken[] { new Token(null), new Token(JAVA_SINGLE_LINE_COMMENT),
			new Token(JAVA_MULTI_LINE_COMMENT), new Token(JAVA_DOC), new Token(JAVA_CHARACTER), new Token(JAVA_STRING),
			new Token(JAVA_STRING), // regular expression same as string
			new Token(JAVA_SHEBANG_LINE), new Token(JAVASCRIPT_TEMPLATE_LITERAL), new Token(JSX) };
	private boolean startQuoteInsideJsx;

	public FastTypeScriptPartitionScanner() {
		// create the scanner
	}
	...
	public IToken nextToken() {
		...
		while (true) {
			...
			case JAVASCRIPT:
				...
				case '/':
					if (fLast == SLASH) {
						...
						switch (lastNonWhitespaceChar) // possible chars before
														// regexp
						{
						case 0: // No char before (the very beginning of a
								// javascript
						case '(':
						case ',':
						case '=':
						...
						fTokenLength++;
						if (fLast == OPEN_BRACKET) {
							fLast = OPEN_BRACKET_AND_SLASH;
						} else {
							fLast = SLASH;
						}
						break;
				...
				default:
					if ((fLast == OPEN_BRACKET || fLast == OPEN_BRACKET_AND_SLASH) && Character.isJavaIdentifierPart(currentChar)) {
						int prefixLength = fLast == OPEN_BRACKET_AND_SLASH ? 3 : 2;
						if (fTokenLength - getLastLength(fLast) > 0)
							return preFix(JAVASCRIPT, JSX_CONTENT, NONE, prefixLength);
						else {
							preFix(JAVASCRIPT, JSX_CONTENT, NONE, prefixLength);
							fTokenOffset += fTokenLength;
							fTokenLength = fPrefixLength;
							break;
						}
			...
			case JSX_CONTENT:
				switch(currentChar) {
				case '"':
					startQuoteInsideJsx = !startQuoteInsideJsx; 
					consume();
					break;
				case '>':
					if (startQuoteInsideJsx) {
						consume();
					} else {
						return postFix(JSX_CONTENT);
					}
					break;
				default:
					consume();
	...
	private static int getState(String contentType) {
		...
		else if (contentType.equals(JSX))
			return JSX_CONTENT;
