...
public class JSDocAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	 ...
	 */
	public JSDocAutoIndentStrategy(String partitioning) {
				...
				if (d.getChar(firstNonWS) == '/' && d.getChar(firstNonWS + 1) != '/') {
					...
					if (isPreferenceTrue(PreferenceConstants.EDITOR_CLOSE_JAVADOCS) && isNewComment(d, offset)) {
						...
						if (isPreferenceTrue(PreferenceConstants.EDITOR_ADD_JAVADOC_TAGS)) {
							...
							IIDETypeScriptFile tsFile = getTypeScriptFile(d);
							if (tsFile != null) {
								try {
									int start = offset - JSDOC_STARTS_LENGTH;
									// We need to remove "/**\r\n" to consume "docCommentTemplate" tsserver command.
									d.replace(start, JSDOC_STARTS_LENGTH, "");
									// Consume "docCommentTemplate" tsserver.
									TextInsertion insertion = tsFile.docCommentTemplate(start).get(5000,
											TimeUnit.MILLISECONDS);
									// Update the Eclispe DocumentCommand with tsserver TextInsertion response. 
									c.caretOffset = insertion.getCaretOffset() + start;
									c.text = insertion.getNewText();
									c.offset = start;
									return;
								} catch (Exception e) {									
									e.printStackTrace();
	...
	private IIDETypeScriptFile getTypeScriptFile(IDocument document) {
		IFile scriptFile = TypeScriptResourceUtil.getFile(document);
		if (scriptFile == null) {
			return null;
		}
		if (TypeScriptResourceUtil.canConsumeTsserver(scriptFile)) {
			try {
				...
				return tsProject.openFile(scriptFile, document);
	...
	private String createJavaDocTags(IDocument document, DocumentCommand command, String indentation,
			String lineDelimiter, IJavaScriptUnit unit) throws CoreException, BadLocationException {
	 ...
	 */
	private String prepareTemplateComment(String comment, String indentation, IJavaScriptProject project,
