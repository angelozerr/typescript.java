...
class EditTemplateDialog extends StatusDialog {
	...
	public EditTemplateDialog(Shell parent, Template template, boolean edit, boolean isNameModifiable,
		...
		String title = edit ? JSDTTypeScriptUIMessages.EditTemplateDialog_title_edit
				: JSDTTypeScriptUIMessages.EditTemplateDialog_title_new;
		setTitle(title);
	...
	protected Control createDialogArea(Composite ancestor) {
		...
		if (fIsNameModifiable) {
			createLabel(parent, JSDTTypeScriptUIMessages.EditTemplateDialog_name);
			...
			createLabel(composite, JSDTTypeScriptUIMessages.EditTemplateDialog_context);
			...
			fAutoInsertCheckbox = createCheckbox(composite, JSDTTypeScriptUIMessages.EditTemplateDialog_autoinsert);
			fAutoInsertCheckbox.setSelection(fTemplate.isAutoInsertable());
		}

		createLabel(parent, JSDTTypeScriptUIMessages.EditTemplateDialog_description);
		...
		Label patternLabel = createLabel(parent, JSDTTypeScriptUIMessages.EditTemplateDialog_pattern);
		...
		fInsertVariableButton.setText(JSDTTypeScriptUIMessages.EditTemplateDialog_insert_variable);
	...
	private void updateStatusAndButtons() {
		...
		if (!fSuppressError && isEmpty) {
			...
			status.setError(JSDTTypeScriptUIMessages.EditTemplateDialog_error_noname);
		} else if (fNameText != null && fNameText.getText().indexOf(' ') > -1) {
			status = new StatusInfo();
			status.setError(JSDTTypeScriptUIMessages.EditTemplateDialog_error_spaces);
		}
		updateStatus(status);
	}
