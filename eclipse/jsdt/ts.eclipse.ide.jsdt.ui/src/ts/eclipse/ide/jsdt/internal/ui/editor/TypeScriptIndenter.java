...
/**
 * Uses the {@link org.eclipse.wst.jsdt.internal.ui.text.JavaHeuristicScanner}
 ...
 * 
 */
public final class TypeScriptIndenter {
	...
	private final class CorePrefs {
		...
		private final ITypeScriptFile fProject;
		...
		CorePrefs(ITypeScriptFile project) {
			fProject = project;
			if (isStandalone()) {
				prefUseTabs = true;
				prefTabSize = 4;
		...
		private boolean prefUseTabs() {
			if (fProject != null) {
				return !fProject.getFormatOptions().getConvertTabsToSpaces();
			}
			return PreferencesHelper.getWorkspacePreferences(TypeScriptCorePlugin.PLUGIN_ID).getBoolean(
		...
		private int prefTabSize() {
			if (fProject != null) {
				return fProject.getFormatOptions().getTabSize();
			}
			return PreferencesHelper.getWorkspacePreferences(TypeScriptCorePlugin.PLUGIN_ID).getInt(
		...
		private int prefIndentationSize() {
			if (fProject != null) {
				return fProject.getFormatOptions().getIndentSize();
			}
			return PreferencesHelper.getWorkspacePreferences(TypeScriptCorePlugin.PLUGIN_ID).getInt(
	 ...
	 *            <code>IDocument</code>.
	 */
	public TypeScriptIndenter(IDocument document, JavaHeuristicScanner scanner) {
	 ...
	 * 
	 */
	public TypeScriptIndenter(IDocument document, JavaHeuristicScanner scanner, ITypeScriptFile project) {
	...
	private StringBuffer createReusingIndent(StringBuffer buffer, int additional) {
		 ...
		 * tabs= tabSize > 0 ? missing / tabSize : 0; spaces= tabSize > 0 ?
		 * missing % tabSize : missing; } else { Assert.isTrue(false); return
		 * null; }
		 */
	 ...
	 * block</li>
	 * <li>opening curly braces are aligned correctly with the introducing
	 * code</li>
				...
				case Symbols.TokenLBRACE: // for opening-brace-on-new-line style
					if (bracelessBlockStart && !fPrefs.prefIndentBracesForBlocks)
						unindent = true;
							...
							|| prevToken == Symbols.TokenRBRACKET) && !fPrefs.prefIndentBracesForArrays)
						unindent = true;
	 ...
	 * <code>Symbols.TokenLBRACE</code>, and <code>Symbols.TokenLBRACKET</code>.
	 * Returns as the reference position either the token introducing the scope
	 * or - if available - the first java token after that.
	 *
	 * <p>
