...
public final class TypeScriptIndenter {
	...
	private final class CorePrefs {
		...
		private boolean prefUseTabs() {
			return !fProject.getFormatOptions().getConvertTabsToSpaces(); //!JavaScriptCore.SPACE.equals(getCoreFormatterOption(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR));
		}

		private int prefTabSize() {
			return fProject.getFormatOptions().getTabSize();//CodeFormatterUtil.getTabWidth(fProject);
		}

		private int prefIndentationSize() {
			return fProject.getFormatOptions().getIndentSize(); //CodeFormatterUtil.getIndentWidth(fProject);
		}
