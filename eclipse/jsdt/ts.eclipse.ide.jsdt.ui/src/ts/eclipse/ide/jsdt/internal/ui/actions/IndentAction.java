...
import ts.eclipse.ide.jsdt.internal.ui.editor.TypeScriptIndenter;
 ...
 * heuristics. The algorithm used is fast for single lines, but does not store
 ...
 * @see org.eclipse.wst.jsdt.internal.ui.text.TypeScriptIndenter
 * 
 */
public class IndentAction extends TextEditorAction {
	...
	public void run() {
		...
		if (document != null) {
			...
			try {
				...
				TypeScriptUIPlugin.log(new Status(IStatus.ERROR, TypeScriptUIPlugin.PLUGIN_ID, IStatus.OK, "", e)); //$NON-NLS-1$
				return;
			}
			...
			Runnable runnable = new Runnable() {
				public void run() {
					...
					try {
						...
						TypeScriptIndenter indenter = new TypeScriptIndenter(document, scanner, getJavaProject());
						...
						// will only happen on concurrent modification
						TypeScriptUIPlugin.log(new Status(IStatus.ERROR, TypeScriptUIPlugin.PLUGIN_ID, IStatus.OK,
	 ...
	 */
	private boolean indentLine(IDocument document, int line, int caret, TypeScriptIndenter indenter,
			JavaHeuristicScanner scanner, boolean multiLine) throws BadLocationException {
		...
		if (offset < document.getLength()) {
			ITypedRegion partition = TextUtilities.getPartition(document, IJavaScriptPartitions.JAVA_PARTITIONING,
					offset, true);
			ITypedRegion startingPartition = TextUtilities.getPartition(document,
					IJavaScriptPartitions.JAVA_PARTITIONING, offset, false);
			String type = partition.getType();
			if (type.equals(IJavaScriptPartitions.JAVA_DOC)
					|| type.equals(IJavaScriptPartitions.JAVA_MULTI_LINE_COMMENT)) {
				indent = computeJavadocIndent(document, line, scanner, startingPartition);
					...
					&& startingPartition.getType().equals(IJavaScriptPartitions.JAVA_SINGLE_LINE_COMMENT)) {
	...
	private String getTabEquivalent() {
		String tab;
		if (getJavaProject().getFormatOptions().getConvertTabsToSpaces()) {
			// if
			// (JavaScriptCore.SPACE.equals(getCoreFormatterOption(DefaultCodeFormatterConstants.FORMATTER_TAB_CHAR)))
			// {
			int size = getTabSize();
			StringBuffer buf = new StringBuffer();
	...
	private int getTabSize() {
		return getJavaProject().getFormatOptions().getTabSize();// getCoreFormatterOption(DefaultCodeFormatterConstants.FORMATTER_TAB_SIZE,
																// 4);
	}

	/**
	 * Returns <code>true</code> if empty lines should be indented, false
	 * otherwise.
	 * 
	 * @return <code>true</code> if empty lines should be indented, false
	 *         otherwise
	 * 
	 */
	private boolean indentEmptyLines() {
		return true; // DefaultCodeFormatterConstants.TRUE.equals(getCoreFormatterOption(DefaultCodeFormatterConstants.FORMATTER_INDENT_EMPTY_LINES));
	}

	/**
	 * Returns the <code>IJavaScriptProject</code> of the current editor input,
	 * or <code>null</code> if it cannot be found.
	 * 
	 * @return the <code>IJavaScriptProject</code> of the current editor input,
	 *         or <code>null</code> if it cannot be found
	 * 
	 */
	private ITypeScriptFile getJavaProject() {
		ITextEditor editor = getTextEditor();
		if (editor == null)
			return null;

		IFile file = EditorUtils.getFile(editor);
