...
public class JSONListProperty extends SimpleListProperty {
	 ...
	 * @param elementType
	 */
	public JSONListProperty(IJSONPath propertyDescriptor,
			Class elementType) {
		...
		if (value instanceof IJSONArray) {
			...
			for (int i = 0; i < array.getLength(); i++) {
				try {
					...
					if (n.getNodeType() == IJSONNode.VALUE_STRING_NODE) {
						IJSONStringValue s = (IJSONStringValue) n;
						result.add(s.getSimpleValue().toString());
					}
				} catch (Exception e) {
					return null;
				}
